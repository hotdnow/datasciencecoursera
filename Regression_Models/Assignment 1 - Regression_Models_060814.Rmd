...
The estimated mean figure for automatic transmission vehicles is **`r round(summary(fit1)$coef[1],2)` miles per gallon** and for manual transmission vehicles it is **`r round(summary(fit1)$coef[2],2)` miles per gallon**. 

**Therefore, this model would predict a manual transmission is generally better for improved miles per gallon. More specifically, the estimated increase in miles per gallon when using a vehicle with a manual transmission (versus one with an automatic transmission) is `r round(summary(fit1)$coef[2] - summary(fit1)$coef[1],2)`.**
...
plot_fit1
```
As mentioned above, the estimated mean figure for automatic transmission vehicles is **`r round(summary(fit1)$coef[1],2)` miles per gallon** and for manual transmission vehicles it is **`r round(summary(fit1)$coef[2],2)` miles per gallon**. 
