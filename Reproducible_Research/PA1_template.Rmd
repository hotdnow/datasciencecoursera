...
```{r}
ttl_stps_day <- as.data.frame(tapply(wm_no_NA$steps, INDEX = wm_no_NA$date, FUN = 'sum', na.rm = TRUE))
...
hist(ttl_stps_day$steps, 
     main = "Total Number of Steps Per Day (excluding missing values)", 
...
Based on this data with the missing values removed, the mean for the total number of steps per day is **`r mean_no_NA`**. 

Based on this data with the missing values removed, the median for the total number of steps per day is **`r median_no_NA`**.   
...
plot(mean_stps_per_intvl$interval, mean_stps_per_intvl$avg_steps, type = "l", 
     xlab= "Five-minute time period", 
     ylab = "Avg number of steps",
     main = "Average Number of Steps by Time Interval (excluding missing values)"
)
```

Based on this data with the missing values removed, the five-minute interval that contains the maximum number of average steps per day is **`r max_interval`**. 

### Imputing missing values

Based in the discussion and accompanying code included above, the total number of rows with NAs is **`r numb_of_NAs`**.
...
```{r}
ttl_stps_day_imput <- as.data.frame(tapply(fill_in_NAs$steps_no_NAs, INDEX = fill_in_NAs$date, FUN = 'sum', na.rm = TRUE))
...
```{r, fig.height=6, fig.width=8, fig.align='center'}
hist(ttl_stps_day_imput$steps, 
     main = "Total Number of Steps Per Day (imputing missing values)", 
     xlab = "Total Number of Steps Per Day",
     ylab = "Frequency (Number of Days)",
     breaks = 10,
     xlim = c(0,25000),
...
Based on this data after imputing missing values, the mean for the total number of steps per day is **`r mean_impute`**. You will recall the corresponding mean value computed based on the dataset with missing values removed was **`r mean_no_NA`**. Based on this metric, the **`r numb_of_NAs`** rows of missing data do not seem to have a material impact on the estimates of the total average daily number of steps.

Based on this data after imputing missing values, the median for the total number of steps per day is **`r median_impute`**. You will recall the corresponding median value computed based on the dataset with missing values removed was **`r median_no_NA`**. Based on this metric, the **`r numb_of_NAs`** rows of missing data do not seem to have a material impact on the estimates of the total average daily number of steps.
...
```{r}
mean_stps_per_intvl_imput <- aggregate(fill_in_NAs$steps_no_NAs, 
                            by = list(fill_in_NAs$interval,
                                         fill_in_NAs$day_of_week), 
                            FUN = 'mean', na.rm = TRUE)
...
```{r}
mean_by_day_type <- aggregate(fill_in_NAs$steps_no_NAs, 
                              by = list(fill_in_NAs$day_of_week), 
                              FUN = 'mean', na.rm = TRUE)
...
```{r, fig.height=6, fig.width=8, fig.align='center'}
xyplot(avg_steps ~ as.numeric(interval) | as.factor(weekday_weekend),
       data = mean_stps_per_intvl_imput,
       type = 'l',
       layout = c(1,2), 
       col = c('blue'),
